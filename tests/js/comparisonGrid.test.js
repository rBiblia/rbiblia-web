...
function testComparisonVerseNavigation() {
  ...
  console.assert(
    nav.canGoPrev() === true,
    "Should be able to go prev from verse 5"
  );
  console.assert(
    nav.canGoNext() === true,
    "Should be able to go next from verse 5"
  );
  ...
  console.assert(
    navFirst.canGoPrev() === false,
    "Should NOT be able to go prev from verse 1"
  );
  navFirst.handlePrev();
  console.assert(
    navFirst.getCurrent() === 1,
    "Should stay at verse 1 when trying to go prev"
  );

  // Boundary: last verse
  const navLast = createNavigator(10, 10);
  console.assert(
    navLast.canGoNext() === false,
    "Should NOT be able to go next from last verse"
  );
  navLast.handleNext();
  console.assert(
    navLast.getCurrent() === 10,
    "Should stay at last verse when trying to go next"
  );

  // Single verse chapter
  const navSingle = createNavigator(1, 1);
  console.assert(
    navSingle.canGoPrev() === false,
    "Single verse: cannot go prev"
  );
  console.assert(
    navSingle.canGoNext() === false,
    "Single verse: cannot go next"
  );
...
function testTranslationGrouping() {
  ...
  console.assert(otherTranslations.length === 2, "Should have 2 non-favorites");
...
function testDiffHighlightingAvailability() {
  ...
  console.assert(canHighlight([]) === false, "No texts → cannot highlight");
  console.assert(
