...
function testDisabledWhenEnabledFalse() {
    const nav = createNavigationTracker();
    const handler = createKeyboardHandler(nav.prevChapter, nav.nextChapter, { enabled: false });
...
function testRapidConsecutivePresses() {
    ...
    console.assert(
        calls[0] === 'next' && calls[1] === 'next' && calls[2] === 'next' && calls[3] === 'prev',
...
function testGetSigla() {
    ...
    console.assert(getSigla('unknown') === 'UNKNOWN', 'unknown → UNKNOWN (fallback)');
...
function testDebounceMinLength() {
    ...
    console.assert(shouldDebounce('ab', 3) === '', 'String shorter than minLength should return empty');
    console.assert(shouldDebounce('abc', 3) === 'abc', 'String equal to minLength should pass through');
    console.assert(shouldDebounce('abcd', 3) === 'abcd', 'String longer than minLength should pass through');
    console.assert(shouldDebounce('', 0) === '', 'Empty string with minLength 0 should pass through');
    console.assert(shouldDebounce(42, 3) === 42, 'Non-string value should pass through regardless of minLength');
...
function testUrlParsingLogic() {
    ...
    console.assert(result.translation === 'pl_ubg', 'Translation should be pl_ubg');
    ...
    console.assert(result.language === 'en', 'Unknown language should fallback to en');

    // Minimal path with defaults
    result = parsePathname('/en');
    console.assert(result.translation === 'pl_ubg', 'Missing translation should get default');
